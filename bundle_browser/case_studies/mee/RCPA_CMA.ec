...
theory MtE.
  ...
  theory RCPA_WUF_RCPA.
    ...
    module IdealTag(M:MACa.MAC_Scheme): Enc_Scheme = {
      proc keygen(): eK * mK = {
        var mk;
    ...
    lemma mte_rcpa_real_equiv (E <: SKEa.Enc_Scheme { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa })
                              (M <: MACa.MAC_Scheme { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa, -E })
                              (A <: RCPA_Adversary  { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa, -E, -M }):
      equiv [ INDR_CPA(MacThenEncrypt(E,M),A).main ~ SKEa.RCPA.INDR_CPA(E,RCPAa(M,A)).main
              : ={glob E, glob M, glob A} ==> ={res} ].
proof.
  proc.
  inline *.
  wp.
  call (: ={glob E, glob M} /\ RCPA_Wrap.k{1} = (SKEa.RCPA.RCPA_Wrap.k{2}, RCPAa.mk{2})).
  proc; inline *.
  wp; call (: true); wp; call (: true); wp; skip; smt().
  wp; call (: true); call (: true); skip; smt().
qed.

    lemma mte_rcpa_real_pr (E <: SKEa.Enc_Scheme { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa })
                           (M <: MACa.MAC_Scheme { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa, -E })
                           (A <: RCPA_Adversary  { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa, -E, -M })
                           &m:
      Pr[INDR_CPA(MacThenEncrypt(E,M),A).main() @ &m: res]
      = Pr[SKEa.RCPA.INDR_CPA(E,RCPAa(M,A)).main() @ &m: res].
proof.
  byequiv (mte_rcpa_real_equiv E M A) => //.
qed.

    lemma mte_rcpa_ideal_hop1_equiv (M <: MACa.MAC_Scheme { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa })
                                    (A <: RCPA_Adversary  { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa, -M }):
      islossless M.keygen =>
      islossless M.tag    =>
      equiv [ INDR_CPA(Ideal,A).main ~ INDR_CPA(IdealTag(M),A).main
              : ={glob M, glob A} ==> ={res} ].
proof.
  move=> Mkg_ll Mtag_ll.
  proc.
  call (: true).
  proc; inline *.
  wp; rnd; call{2} Mtag_ll; auto.
  inline *; wp; call{2} Mkg_ll; auto.
qed.

    lemma mte_rcpa_ideal_hop1_pr (M <: MACa.MAC_Scheme { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa })
                                 (A <: RCPA_Adversary  { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa, -M })
                                 &m:
      islossless M.keygen =>
      islossless M.tag    =>
      Pr[INDR_CPA(Ideal,A).main() @ &m: res]
      = Pr[INDR_CPA(IdealTag(M),A).main() @ &m: res].
proof.
  move=> Mkg_ll Mtag_ll.
  byequiv (mte_rcpa_ideal_hop1_equiv M A Mkg_ll Mtag_ll) => //.
qed.

    lemma mte_rcpa_ideal_hop2_equiv (M <: MACa.MAC_Scheme { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa })
                                    (A <: RCPA_Adversary  { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa, -M }):
      equiv [ INDR_CPA(IdealTag(M),A).main ~ SKEa.RCPA.INDR_CPA(SKEa.RCPA.Ideal,RCPAa(M,A)).main
              : ={glob M, glob A} ==> ={res} ].
proof.
  proc.
  inline *.
  wp.
  call (: ={glob M} /\ RCPA_Wrap.k{1}.`2 = RCPAa.mk{2}).
  proc; inline *.
  wp; rnd; wp.
  call (: true); auto.
  swap{2} 1 1.
...
qed.
    ...
    lemma mte_rcpa_ideal_hop2_pr (M <: MACa.MAC_Scheme { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa })
                                 (A <: RCPA_Adversary  { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa, -M })
                                 &m:
      Pr[INDR_CPA(IdealTag(M),A).main() @ &m: res]
      = Pr[SKEa.RCPA.INDR_CPA(SKEa.RCPA.Ideal,RCPAa(M,A)).main() @ &m: res].
proof.
  byequiv (mte_rcpa_ideal_hop2_equiv M A) => //.
qed.

    lemma mte_rcpa_ideal_pr (M <: MACa.MAC_Scheme { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa })
                            (A <: RCPA_Adversary  { -RCPA_Wrap, -SKEa.RCPA.RCPA_Wrap, -RCPAa, -M })
                            &m:
      islossless M.keygen =>
      islossless M.tag    =>
...
proof.
  move=> Mkg_ll Mtag_ll.
  rewrite (mte_rcpa_ideal_hop1_pr M A &m Mkg_ll Mtag_ll).
  apply (mte_rcpa_ideal_hop2_pr M A &m).
qed.
...
(* COMPLETE THIS *)
  move=> Mkg_ll Mtag_ll.
  rewrite (mte_rcpa_real_pr E M A &m) (mte_rcpa_ideal_pr M A &m Mkg_ll Mtag_ll).
  done.
qed.
  end RCPA_WUF_RCPA.
  ...
  theory RCPA_WUF_PTXT.
    ...
    section PTXT.
      ...
      local equiv mte_ptxt_enc_eq: PTXT_Wrap(MacThenEncrypt(E,M)).enc ~ CMAa(E,A,MACa.WUF_CMA.WUF_Wrap(M)).Sim.enc: ={p, glob E, glob M} /\ PTXT_Wrap.k{1} = (CMAa.ek{2}, MACa.WUF_CMA.WUF_Wrap.k{2}) /\ PTXT_Wrap.s{1} = MACa.WUF_CMA.WUF_Wrap.s{2} /\ PTXT_Wrap.win{1} = MACa.WUF_CMA.WUF_Wrap.win{2} ==> ={res, glob E, glob M} /\ PTXT_Wrap.k{1} = (CMAa.ek{2}, MACa.WUF_CMA.WUF_Wrap.k{2}) /\ PTXT_Wrap.s{1} = MACa.WUF_CMA.WUF_Wrap.s{2} /\ PTXT_Wrap.win{1} = MACa.WUF_CMA.WUF_Wrap.win{2}.
proof.
  proc.
  inline *.
  swap{2} 3 2.
...
qed.
      ...
      local equiv mte_ptxt_verify_eq: PTXT_Wrap(MacThenEncrypt(E,M)).verify ~ CMAa(E,A,MACa.WUF_CMA.WUF_Wrap(M)).Sim.verify: ={c, glob E, glob M} /\ PTXT_Wrap.k{1} = (CMAa.ek{2}, MACa.WUF_CMA.WUF_Wrap.k{2}) /\ PTXT_Wrap.s{1} = MACa.WUF_CMA.WUF_Wrap.s{2} /\ PTXT_Wrap.win{1} = MACa.WUF_CMA.WUF_Wrap.win{2} ==> ={res, glob E, glob M} /\ PTXT_Wrap.k{1} = (CMAa.ek{2}, MACa.WUF_CMA.WUF_Wrap.k{2}) /\ PTXT_Wrap.s{1} = MACa.WUF_CMA.WUF_Wrap.s{2} /\ PTXT_Wrap.win{1} = MACa.WUF_CMA.WUF_Wrap.win{2}.
proof.
  proc.
  inline *.
  wp.
  sp 3 1.
  sp 1 0.
  seq 1 1 : (pt{1} = pt{2} /\ (glob E){1} = (glob E){2} /\ (glob M){1} = (glob M){2} /\ mk{1} = MACa.WUF_CMA.WUF_Wrap.k{2} /\ p0{1} = None /\ b{2} = false /\ PTXT_Wrap.k{1} = (CMAa.ek{2}, MACa.WUF_CMA.WUF_Wrap.k{2}) /\ PTXT_Wrap.s{1} = MACa.WUF_CMA.WUF_Wrap.s{2} /\ PTXT_Wrap.win{1} = MACa.WUF_CMA.WUF_Wrap.win{2}).
...
qed.
      ...
      local equiv mte_ptxt_forge_eq: A(PTXT_Wrap(MacThenEncrypt(E,M))).forge ~ A(CMAa(E,A,MACa.WUF_CMA.WUF_Wrap(M)).Sim).forge: ={glob A, glob E, glob M} /\ PTXT_Wrap.k{1} = (CMAa.ek{2}, MACa.WUF_CMA.WUF_Wrap.k{2}) /\ PTXT_Wrap.s{1} = MACa.WUF_CMA.WUF_Wrap.s{2} /\ PTXT_Wrap.win{1} = MACa.WUF_CMA.WUF_Wrap.win{2} ==> ={glob A, glob E, glob M} /\ PTXT_Wrap.k{1} = (CMAa.ek{2}, MACa.WUF_CMA.WUF_Wrap.k{2}) /\ PTXT_Wrap.s{1} = MACa.WUF_CMA.WUF_Wrap.s{2} /\ PTXT_Wrap.win{1} = MACa.WUF_CMA.WUF_Wrap.win{2}.
proof.
  proc (={glob E, glob M} /\ PTXT_Wrap.k{1} = (CMAa.ek{2}, MACa.WUF_CMA.WUF_Wrap.k{2}) /\ PTXT_Wrap.s{1} = MACa.WUF_CMA.WUF_Wrap.s{2} /\ PTXT_Wrap.win{1} = MACa.WUF_CMA.WUF_Wrap.win{2}).
  by move=> &1 &2 />.
  by move=> &1 &2 />.
  conseq mte_ptxt_enc_eq.
  by move=> &1 &2 />.
  by move=> &1 &2 />.
  conseq mte_ptxt_verify_eq.
  by move=> &1 &2 />.
  by move=> &1 &2 />.
qed.

      local equiv mte_ptxt_main_eq: INT_PTXT(MacThenEncrypt(E,M),A).main ~ MACa.WUF_CMA.WUF_CMA(M,CMAa(E,A)).main: ={glob A, glob E, glob M} ==> ={res}.
proof.
  proc.
  inline *.
  swap{2} 4 -3.
  call mte_ptxt_forge_eq.
  wp.
  call (_: true).
  call (_: true).
  skip; smt().
qed.
      ...
      local lemma mte_ptxt_pr_eq &m: Pr[INT_PTXT(MacThenEncrypt(E,M),A).main() @ &m: res] = Pr[MACa.WUF_CMA.WUF_CMA(M,CMAa(E,A)).main() @ &m: res].
proof.
  byequiv mte_ptxt_main_eq=> //.
qed.
