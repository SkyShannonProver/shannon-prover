...
lemma xorbKl (a b : block): (a +^ b) +^ a = b.
proof.
  by rewrite xorbC xorbA xorbK xorbC xorb0.
qed.

lemma cbc_encK (P Pi : block -> block -> block) (ek : block):
  cancel (P ek) (Pi ek) =>
  forall (p : block list) (st : block),
    cbc_dec Pi ek st (cbc_enc P ek st p) = p.
proof.
  move=> hK; elim=> //= pi p ih st.
  by rewrite hK xorbKl ih.
qed.

lemma size_padding_P2 (l : int): 0 <= l => size (padding l) = l.
proof.
  move=> hl; rewrite /padding size_mkseq; smt().
qed.

lemma nth_padding (l i : int): 0 <= i < l =>
  nth witness (padding l) i = int2o l.
proof.
  by move=> hi; rewrite /padding nth_mkseq.
qed.

lemma last_padding_P2 (l : int): 0 < l => last witness (padding l) = int2o l.
proof.
  by move=> gt0_l; rewrite /padding mkseq_nseq last_nseq.
qed.

lemma last_cat_padding (os : octet list) (l : int): 0 < l =>
  last witness (os ++ padding l) = int2o l.
proof.
  move=> gt0_l; rewrite last_cat /padding mkseq_nseq last_nseq //.
qed.

lemma os2tK_P2 (t : tag): os2t (t2os t) = t.
proof.
  rewrite /os2t /t2os -{1}(size_tag t) chunk_exact.
  by rewrite -size_eq0 size_tag.
  by rewrite /= mktagK.
qed.

lemma pad_length_rng (m : msg): 1 <= pad_length m <= 16.
proof.
  rewrite /pad_length; smt(size_ge0 modz_ge0 ltz_pmod).
qed.

lemma o2int_int2o (i : int): 0 <= i < 256 => o2int (int2o i) = i.
proof.
  move=> hi; rewrite /o2int /int2o.
  rewrite ofoctetK 1:size_int2bs //.
  by rewrite int2bsK //=; smt(hi).
qed.

lemma size_cat_pad (m : msg) (t : tag) (l : int): 0 <= l =>
  size (m ++ t2os t ++ padding l) = size m + 32 + l.
proof.
  move=> hl; rewrite !size_cat size_t2os size_padding_P2 //.
qed.

lemma dvd16_size_cat_pad (m : msg) (t : tag):
  16 %| size (m ++ t2os t ++ padding (pad_length m)).
proof.
  have hl : 0 <= pad_length m by smt(pad_length_rng).
  rewrite (size_cat_pad m t (pad_length m) hl).
  rewrite /pad_length; smt(divz_eq dvdzE modzMDl).
qed.

lemma map_ofblockK (oss : octet list list):
  (forall (os : octet list), os \in oss => size os = 16) =>
  map ofblock (map mkblock oss) = oss.
proof.
  elim: oss => [//| os oss ih] h /=.
  split; [by rewrite ofblockK 1:h 1:mem_head | by apply/ih => os' hos'; apply/h; rewrite in_cons hos'].
qed.

lemma bs2osK_P2 (os : octet list): 16 %| size os => bs2os (os2bs os) = os.
proof.
  move=> hdvd; rewrite /bs2os /os2bs -map_comp.
  rewrite -{2}(chunkK 16 os) // -{2}(map_id (chunk 16 os)).
  congr; apply/eq_in_map=> c hc /=.
  by rewrite /(\o) /idfun /= ofblockK 1:(in_chunk_size 16 os).
qed.

lemma bs2os_pad (m : msg) (t : tag):
  bs2os (pad m t) = m ++ t2os t ++ padding (pad_length m).
proof.
  rewrite /pad /=; apply/bs2osK_P2; exact/dvd16_size_cat_pad.
qed.

lemma take_msg_cat_pad (m : msg) (t : tag) (l : int):
  take (size m) (m ++ t2os t ++ padding l) = m.
proof.
  by rewrite -catA take_size_cat.
qed.

lemma take_tag_cat_pad (m : msg) (t : tag) (l : int):
  take 32 (drop (size m) (m ++ t2os t ++ padding l)) = t2os t.
proof.
  rewrite -catA drop_size_cat //.
  by rewrite take_size_cat 1:size_t2os.
qed.

lemma drop_pad_cat_pad (m : msg) (t : tag) (l : int): 0 <= l =>
  drop (size m + 32) (m ++ t2os t ++ padding l) = padding l.
proof.
  by move=> _; apply/drop_size_cat; rewrite size_cat /t2os size_tag.
qed.

lemma unpad_cat_pad (m : msg) (t : tag) (l : int):
  1 <= l <= 16 =>
  16 %| size (m ++ t2os t ++ padding l) =>
  unpad (os2bs (m ++ t2os t ++ padding l)) = Some (m, t).
proof.
  move=> hl hdvd.
  rewrite /unpad bs2osK_P2 1:hdvd /=.
  rewrite last_cat_padding 1:/# o2int_int2o 1:/#.
  rewrite (size_cat_pad m t l) 1:/#.
  have ->: size m + 32 + l - l = size m + 32 by smt().
  have ->: size m + 32 - 32 = size m by smt().
  rewrite drop_pad_cat_pad 1:/# take_msg_cat_pad take_tag_cat_pad os2tK_P2.
  by rewrite hl.
qed.

lemma unpad_pad (m : msg) (t : tag): unpad (pad m t) = Some (m, t).
proof.
  rewrite /pad /=.
  apply/unpad_cat_pad; [exact/pad_length_rng | exact/dvd16_size_cat_pad].
qed.

lemma mee_dec_enc_unpad (P Pi : block -> block -> block) (M : mK -> msg -> tag)
                        (ek : block) (mk : mK) (iv : block) (m : msg):
  cancel (P ek) (Pi ek) =>
  unpad (cbc_dec Pi ek iv (mee_enc P M ek mk iv m)) = Some (m, M mk m).
proof.
  move=> hK.
  rewrite /mee_enc /= (cbc_encK P Pi ek hK) unpad_pad //.
qed.
...
(* COMPLETE THIS *)
  move=> hK.
  rewrite /mee_dec /= (mee_dec_enc_unpad P Pi M ek mk iv m hK) /=.
  done.
qed.
