"""Proof-node bootstrap and replay bookkeeping state.
...
projection bookkeeping.  Renamed from ``lifecycle.py``: it never touches OS
processes, so the old name misleadingly mirrored ``workflow/proc_lifecycle.py``
