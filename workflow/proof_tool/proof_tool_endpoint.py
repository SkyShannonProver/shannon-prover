"""Authenticated loopback endpoint for the managed proof tool.
...
It delegates every admitted call to :mod:`workflow.proof_tool.proof_tool_session` without
parsing or validating proof intents.
"""
    ...
    def _handle_call(self, envelope: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not ready_valid:
            return self._error(
                ...
                "proof-tool invocation has no exact READY registration",
            )

        raw_identity = envelope.get("tool_identity")
        if not isinstance(raw_identity, Mapping):
            return self._error(
                "invalid_tool_identity", "proof-tool identity must be an object"
            )
        try:
            ...
            return self._error("invalid_tool_identity", str(exc))
        ...
        try:
            ...
            return self._error(
                "invalid_deadline", "proof-tool absolute deadline must be numeric"
            )
        if not math.isfinite(absolute_deadline) or absolute_deadline <= 0:
            return self._error(
                "invalid_deadline", "proof-tool absolute deadline is invalid"
            )
