...
- the per-turn token-watermark detector (Layer 1 detection), and
- the frontier brief + accepted-spine builder (the handoff).

(The Layer-2 proof-open/premature-give-up classifier was removed; Layer 1 plus
the supervisor's Layer-3 crash net are the current mechanism.)

The wiring that *acts* on these decisions lives in
``workflow.node.proof_node_runtime`` (the in-worker swap) and
``workflow.tree.supervisor`` (the Layer-3 crash net). Keeping the decision
...
class CtxWatermarkDetector:
    ...
    def observe(self, event: dict[str, Any]) -> bool:
        """Update state for one Claude `assistant` event; see observe_tokens."""
        return self.observe_tokens(context_tokens_from_assistant_event(event))

    def observe_tokens(self, ctx: int | None) -> bool:
        """Update state for one turn's context size; True iff pressure just tripped.

        Provider-neutral core: Claude feeds it via
        :func:`context_tokens_from_assistant_event`, Codex via
        :func:`context_tokens_from_codex_event`. The trip is a rising edge —
        returns True only on the transition into pressure, so the caller can
        end the generation exactly once, between turns.
        """
