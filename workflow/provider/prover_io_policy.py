...
_RUNTIME_BRIDGE_DIRECT_RE = re.compile(
    # Layer packages sit between `workflow` and the module now
    # (workflow.proof_tool.proof_node_mcp_server); keep matching any depth.
    r"\bworkflow(?:\.[a-z_]+)*\.proof_node_(?:runtime_(?:client|cli)|mcp_server)\b"
)
...
def classify_read_path(
    ...
    *,
    ...
    if _is_allowed_source_file(resolved, cwd=cwd, allowed_source_files=allowed_source_files):
        safety, safety_detail = _target_source_answer_safety(resolved, target_lemma)
        if safety == "target_source_unreadable":
            # Fail closed (the answer safety of the file cannot be verified),
            # but as an environment error — never as an answer-leak attempt.
            return InformationSourceDecision(
                decision="deny",
                source_type="active_target_source_unreadable",
                authority="source_text",
                reason=(
                    "The active target source file could not be read to verify "
                    f"answer safety ({safety_detail}). This is an environment or "
                    "permission problem, not a leak attempt; fix file access or "
                    "use the manager workspace view."
                ),
                audit_code="read.active_target_source_unreadable",
            )
    ...
    if run_source_root is not None and (
        ...
        if resolved.suffix in (".ec", ".eca") and (
            _target_source_answer_safety(resolved, target_lemma)[0]
    ...
    if (eval_mode and target_lemma and resolved.suffix in (".ec", ".eca")
            and _target_source_answer_safety(resolved, target_lemma)[0]
            == "contains_substantive_target_proof"):
        return InformationSourceDecision(
            decision="deny",
...
def destructive_tool_denylist(
    ...
    *,
    ...
    home = Path.home()
    # Framework files now live in layer subpackages (workflow/node,
    # workflow/proof_tool, workflow/provider); `**` matches any depth, and a
    # basename-anchored Bash rule below also catches relative-path access
    # (the old full-path substring rule missed `cat workflow/x.py`).
    framework_files = [
    ...
    patterns.extend(
        f"Bash(*{name}*)" for name in sorted(_FRAMEWORK_ESCAPE_FILES)
    )
    # FRAMEWORK-CODE WRITE BOUNDARY (observed live 2026-08-19): a prover whose
    # intents kept failing EDITED workflow/proof_management/protocol_repair.py
    # to "fix" the manager mid-run. The agent must never modify its own
    # referee, the test suite that vouches for it, or any EasyCrypt source
    # (sibling-lemma edits corrupt the axiom environment; verified write-back
    # is harness-owned). Reads outside the escape-file list stay allowed for
    # source context; run artifacts (node-memory notes) are not .py/.ec files
    # and stay writable.
    framework_write_trees = [
        f"{root}/core/**",
        f"{root}/workflow/**/*.py",
        f"{root}/tests/**",
        f"{root}/scripts/**",
    ...
    for tool in ("Edit", "MultiEdit", "Write", "NotebookEdit"):
        for pattern in framework_write_trees:
            patterns.append(_abs_fs_rule(tool, pattern))
        for pattern in framework_write_rel:
            patterns.append(f"{tool}({pattern})")
...
def _target_source_answer_safety(
    path: Path, target_lemma: str | None
) -> tuple[str, str]:
    """Classify whether exposing the active target source could leak the answer.

    Returns ``(safety, detail)``. ``target_source_unreadable`` (an environment
    problem — permissions, missing file) is deliberately distinct from
    ``unverified_target_source`` (the lemma is absent from the text): the
    former must never be audited as an answer-leak attempt.
    """
    lemma = str(target_lemma or "").strip()
    if not lemma:
        return "unverified_target_source", "no target lemma set"
    try:
        text = path.read_text(encoding="utf-8", errors="replace")
    except OSError as exc:
        return "target_source_unreadable", f"{type(exc).__name__}: {exc}"
    body = _target_lemma_proof_body(text, lemma)
    if body is None:
        return "unverified_target_source", f"lemma {lemma!r} not found in file"
