...
``workflow/tree/trackers.py:_assistant_context_before_tool`` (only size/hash/markers).
...
Usage:
    python3 -m workflow.reporting.agent_thinking_trace <run_iteration_dir>
    python3 -m workflow.reporting.agent_thinking_trace --node <node_memory/Tree_x> \
...
from workflow.reporting.replay_artifacts import node_id_from_dir as _node_id_from_dir
from typing import Any
from workflow.provider.codex_event_normalizer import codex_event_text
...
def _extract_codex_turns(
    ...
    for event in events:
        ...
        if item_type == "mcp_tool_call" and event_type in {
            ...
            if (
                server not in {"", PROOF_TOOL_IDENTITY.server}
                or tool != PROOF_TOOL_IDENTITY.tool
            ):
                continue
