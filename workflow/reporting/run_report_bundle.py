...
Usage (also hooked into eval_suite.run / orchestrator post-run):
    python3 -m workflow.reporting.run_report_bundle <run_iteration_dir> \
...
def _session_history_exact(run_iter: Path, tree: str) -> list[str]:
    ...
    """
    tree_component = str(tree or "").strip().lower()
    if re.fullmatch(r"tree_[a-z0-9_]+", tree_component) is None:
        return []
    archive_root = run_iter / "ec_sessions"
    if not archive_root.is_dir():
        return []
    session_name = re.compile(
        rf"\.ec_session_prover_(?:[a-z0-9]{{6,32}}_)?"
        rf"{re.escape(tree_component)}"
    )
    matches = [
        path
        for path in archive_root.iterdir()
        if path.is_dir()
        and session_name.fullmatch(path.name) is not None
        and _session_identity_matches_run(run_iter, path)
    ]
    # A report adapter must never pick a session by recency or size. Multiple
    # identity-compatible archives are an ambiguous provenance failure.
    if len(matches) != 1:
        return []
    return _read_history_file(matches[0] / "history.ec")
...
def _build_chain_bundle(
    ...
    *,
    ...
    for label, ci in zip(labels, chain):
        try:
            from workflow.reporting.agent_thinking_trace import write_run_thinking
    ...
    try:
        subprocess.run(
            [sys.executable, "-m", "workflow.reporting.agent_view_timeline_report",
